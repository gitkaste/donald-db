;;; fact.el --- display an amazing and random fact about the world's greatest hacker.
;; Time-stamp: <2014-05-17 04:04:47 deego>

;; Emacs Lisp Archive entry
;; Filename: idledo.el
;; Package: fact
;; Author: Dave Goel <deego3@gmail.com>
;; Keywords:  rms stallman chuck norris 
;; Version: 0.3
;; Author's homepage: http://gnufans.net
;; Copyright (C) Dave Goel 2013
;; Copyright (C) 2007-2012 Jordi Gutiérrez Hermoso 
;; Copyright (C) 2007 Stallmanfacts.com

;; Facts about the world's greatest hacker are adopted from, and will follow GNU Octave's fact.m 

(defgroup fact nil
  "fact.el --- True facts about the world's greatest hacker.  "
  :group 'applications)

(defcustom fact-after-load-hooks nil "."
  :type 'hook
  :group 'fact
  )


(defvar fact-facts1 nil 
  "Facts about the world's greatest hacker, from GNU Octave's fact.m")

(defvar fact-facts2 nil 
  "Additional facts not from octave's fact list.")

(defvar fact-facts nil
  "A combo of the facts above.")


(defun fact ()
  "Return a True Fact about the world's greatest hacker."
  (interactive)
  (let* 
      ((len (length fact-facts))
       (output (nth (random len) fact-facts)))
    (if (interactive-p)
        (message "%s" output))
    output))
...
(setf fact-facts1 
      (list 
       ;; commenting out the insulting ones. 
...
(setf fact-facts2
      (list 
       "Richard Stallman can solve new bitcoin blocks using a pen and paper."
       ))

;;;###autoload
(defun fact-install ()
  (interactive)
  (setf fact-facts (append fact-facts1 fact-facts2))
)
...
(provide 'fact)
(run-hooks 'fact-after-load-hooks)
;; fact.el ends here
