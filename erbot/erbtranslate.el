...
(defun fsi-translate-old-via-translatedotel (&optional from to &rest text)
...
(defun fsi-translate (&optional from to &rest text)
  (erbtranslate-enabled-check)
  (sit-for 0.2) ;; to guard against multi-abuse scripts..
  (if (not (and from to text))
      (let ((frame (backtrace-frame 3)) (caller nil))
        (setq caller (symbol-name (cadr frame))
              caller (replace-regexp-in-string "^\\w+-" "" caller))
    ...
    (setq text (mapconcat #'(lambda (arg) (format "%s" arg)) text " ")
          from (format "%s" from)
          to   (format "%s" to  ))
    ;; just an extra measure of safety. Should not really be needed:
    (assert (string-match "^[a-zA-Z]+$" from))
    (assert (string-match "^[a-zA-Z]+$" to))
    (erbutils-ignore-errors
     (aref (erbtranslate-via-gt from to text)
	   6))))


(defun erbtranslate-via-gt (src dest txt)
  "New. 20141114dg: Translate txt from src to dest, return a json output. Usually, you'd want aref 6 of this output..."
  ;; (error "in development.")
  ;; this stuff is copied from google-translate-translate and modified.
  (require 'google-translate)
  (let* ((json (google-translate-request src
                                         dest
                                         txt))
	 (detailed-translation nil))
    (if (null json)
	nil
      (make-gtos
       :source-language src
       :target-language dest
