...
(defvar erbn-pf-file-verbatim "~/public_html/data/userfunctions.el-raw")
...
(defmacro fsi-defun (fcn args &rest body)
  ...
  (let ((docp nil))
    ...
    (when (functionp fcn)
      (error 
       "%s is already a function. ,ff it, EXAMINE IT, and if applicable, fmakunbound it first. "  fcn))
...
(erbutils-defalias-f '(type-of pp-to-string))
