...
(defun erbim-where-is-char (c &optional im-list)
...
(defun fsi-where-is-char (&optional key &rest im-list)
  (let ((imlist nil) 
    ...
    (if key (erbim-where-is-char key (mapcar 'symbol-name im-list)) 
      "where-is-char <CHAR-OR-SEQUENCE> [ INPUT-METHOD INPUT-METHOD... ]") ))
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;; look for X Compose mappings
(defun erbim-where-is-composed-char-file (locale)
...
(defun erbim-where-is-composed-char (c &optional locale)
  (or locale (setq locale "en"))
  (let ((compose-file (erbim-where-is-composed-char-file locale)) 
  ...
  (let ((key (if key (if (symbolp key) (aref (symbol-name key) 0) key) nil))
    ...
    (if (not key)
      ...
      (mapc
       ...
       (erbim-where-is-composed-char key locale))
